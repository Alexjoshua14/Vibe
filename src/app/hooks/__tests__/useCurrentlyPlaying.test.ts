...
describe('Testing useCurrentlyPlaying', () => {
  ...
  beforeAll(() => {
    mockedGetCurrentlyPlaying = 
      vi.spyOn(spotifyAPI, 'getClientCurrentlyPlaying')
        .mockImplementation(async () => Promise.resolve(currentlyPlaying) )

    vi.useFakeTimers()
    setIntervalSpy = vi.spyOn(global, 'setInterval')
    dateSpy = vi.spyOn(global.Date, 'now').mockReturnValue(currentlyPlaying.timestamp + 10000)
  ...
  it('Check that currentlyPlaying song is updated on fetch', () => {
    expect(renderedHook.result.current.currentlyPlaying).toEqual(currentlyPlaying)
  })
  ...
  it('Ensure that data fetching has been scheduled to run repeatedly', () => {
    ...
    expect(setIntervalSpy).toHaveBeenNthCalledWith(1, expect.any(Function), 10000)
