...
import { createSession, getQueueSession } from "@/lib/queue-session/session-management"
...
export default async function SharedQueue({ params }: pageProps) {
  ...
  const currentlyPlayingId = await createSession()
  let currentlyPlaying

  if (currentlyPlayingId) {
    currentlyPlaying = await getQueueSession(currentlyPlayingId)
  } else {
    console.log("Failed to create currently playing..")
  }
