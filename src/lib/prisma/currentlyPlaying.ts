...
export async function addUserToSession(sessionId: string) {
  ...
  const cp = await prisma.currentlyPlaying.findFirst({ where: { id: sessionId }, select: { members: true }})
...
export async function reconnect() {
  ...
  const userDB = await prisma.user.findFirst({where: {id: user.user.id}, include: { currentlyPlaying: true }})
  ...
  const prevStatus: 'HOST' | 'MEMBER' = userDB?.currentlyPlaying.userId === user.user.id ? 'HOST' : 'MEMBER'
